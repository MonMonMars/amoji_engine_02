...
/** Build lip-sync parameters from assistant audio. */
export function lipSyncParameters(pcm16: Int16Array): FaceLiveParameter[] {
  const mouthOpen = mouthOpenFromPcm16(pcm16);
  return [
...
export function inferExpressionFromText(text: string): SakuraExpression {
  ...
  if (/哈哈|開心|好呀|正|掂|thank|thanks|great/.test(lower)) return "happy";
  if (/哇|嘩|唔信|真係/.test(text) || /!{2,}|！{2,}/.test(text)) return "surprised";
  if (/唉|唔開心|傷心|sorry|sad/.test(lower)) return "sad";
  if (/諗|思考|點解|why|hmm/.test(lower)) return "thinking";
  return "neutral";
}
