...
Lab: **Motion:** button cycles vendors (`?motion=` / `amoji.motionVendor`); **Motion demo** / `V` cycles talk styles into the active vendor and posts begin → animated frames (~12 Hz) → end to `?motionBridge=` while idle (Gesture-demo parity — no voice turn required). SoftBank turns emit `annotatedReply` on the `sakura` begin event; Furhat turns emit `sayText` / `furhat.say` for `POST /furhat/say`. Robot HUD **motion** row shows the active package + dispatch log.
