 ...
 * Values are small XYZ Euler on the normalized humanoid.
 */
export const COMPANION_FINGER_POSE_SCHEMA = "amoji.companionFingerPose.v1";
...
export function applyFingerRestPose(applyBone, opts = {}) {
  ...
  for (const name of VRM_FINGER_BONE_NAMES) {
    ...
    applyBone(name, {
      x: rest.x,
      y: rest.y,
      z: rest.z + sign * (name.includes("Thumb") ? boost * 0.35 : boost),
    });
    applied += 1;
  }
  return applied;
