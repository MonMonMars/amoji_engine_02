...
export function createCompanionBodyMotion(humanoid, opts = {}) {
  ...
  const applyPlantedForearmIdle = (pose, k = 1) => {
    const restLl = armRestRotations.leftLowerArm;
    const restRl = armRestRotations.rightLowerArm;
    const apose = isAposeBind();
  ...
  const applyPlantedArmPresentation = () => {
    ...
    applyPlantedForearmIdle(smoothedPose, 1);
  };
  ...
  const applyPose = (pose, intensity = 1, opts = {}) => {
    ...
    if (eatArms) {
      ...
      if (opts.plantFeet !== false && !talking && !opts.idleBeatArms) {
        ...
        if (opts.plantedRestSnap) {
          ...
          applyPlantedForearmIdle(pose, k);
        }
      } else {
    ...
    const plantedShoulders =
      (idleArms ||
        (talkArmBlend > 0.01 &&
          opts.plantFeet !== false &&
          !actionArms &&
          !allowArms &&
          !eatArms)) &&
      !opts.idleBeatArms;
    if (leftShoulder && !plantedShoulders) {
      leftShoulder.rotation.z = (pose.shoulderL || 0) * k;
    } else if (plantedShoulders) {
      writeHumanoidBoneRotation(humanoid, "leftShoulder", PLANTED_SHOULDER_REST);
      writeHumanoidBoneRotation(humanoid, "rightShoulder", PLANTED_SHOULDER_REST);
    }
    if (rightShoulder && !plantedShoulders) {
